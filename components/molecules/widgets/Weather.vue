...
const { locations,selectedLocation, weather } = storeToRefs(widgetsStore)
const selected = selectedLocation

const chooseLocation = async (location: string) => {
  await widgetsStore.fetchWeatherByLocation(location);
};
...
<template>
  <ClientOnly>
    <div class="h-[400px] mt-4 rounded relative overflow-hidden">
      <img src="https://images.seattletimes.com/wp-content/uploads/2023/03/03132023_Weather_075056.jpg?d=780x520"
        class="w-full h-full object-cover" />
      <div
        class="absolute inset-0 z-10 bg-neutral-6 bg-opacity-50 w-80 cursor-pointer border rounded flex flex-col justify-center items-center text-center p-13.5 mx-auto text-white backdrop-blur-xs">
        <div class="text-md font-bold flex flex-col gap-2 mb-1 w-full">
          <div class="mt-4">
            <div id="location" class="relative">
                <HeadlessListbox v-model="selected">
                  <div class="relative mt-1">
                    <HeadlessListboxButton
                      class="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left text-black shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
                      <span class="block truncate">{{ selected }}</span>
                      <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                        <Icon name="ph:caret-up-down-light" class="h-5 w-5 text-gray-400" aria-hidden="true" />
                      </span>
                    </HeadlessListboxButton>
                    ...
                    <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                      ...
                      <HeadlessListboxOptions
                        class="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white text-base text-left shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                        <HeadlessListboxOption v-slot="{ active, selected }" v-for="(location, index) in locations" :value="location" :key="index"
                           as="template">
                          <li :class="[
          ...
          <span class="uppercase">Hôm nay</span>
          <span class="font-normal text-sm">
            {{ weather?.location?.localtime.split(" ")[0] }}</span>
        </div>
        ...
        <p class="mb-2">Partly cloud</p>
        <div class="text-3xl font-bold mb-6 flex items-center gap-2">
          {{ weather?.current?.temp_c }}°C<span class="font-normal mx-1">/</span>{{ weather?.current?.temp_f }}°F
        </div>
        <div class="flex justify-between w-full">
          <div class="flex items-center px-2">
            ...
            {{ weather?.current?.humidity }} l/m<sup>2</sup>
          </div>
          <div class="flex items-center px-2">
