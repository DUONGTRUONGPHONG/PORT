...
// console.log("cmt", comments, props.user)
</script>
<template>
  <ClientOnly>
    <div class="my-2" v-for="(item, index) in comments.items" :key="index">
      <Row :class="['my-4', type === 'reply' ? 'pl-4' : '']">
        <div class="w-full">
          <Row class="flex">
            <CommentsAvatar :payload="{ id: item.readerId, userName: item?.userName, avatar: item?.avatar }"
              class="mr-2" />
            <Row>
              <span class="text-sm font-semibold whitespace-nowrap">
                {{ user && user?.userId === item?.readerId ? 'Tôi' : item?.userName }}
              </span>
              <span class="text-sm pl-1">{{ item?.content }}</span>
              <!-- <a-popover placement="right" trigger="click">
                <template #content>
                  <div class="min-w-[200px]">
                    <div class="px-2 py-1 cursor-pointer hover:bg-[#E5E5E5] rounded-sm">Chỉnh sửa</div>
                    <div class="px-2 py-1 cursor-pointer hover:bg-[#E5E5E5] rounded-sm">Xóa</div>
                  </div>
                </template>
                <a-button class="ml-2">
                  <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" class=""
                    style="--color: #050505;">
          ...
          <div v-if="replyingTo === item.id" class="relative pl-10">
            <textarea v-model="commentContent"
              class="w-full min-h-[42px] h-16 placeholder:text-sm p-2 border-solid border outline-none border-stone-200 rounded"
            ...
            <div class="absolute right-0 bottom-3">
              <button class="flex items-center mr-2 p-2 bg-blue text-[#fff] rounded-xl text-xs"
                @click="!activeButtonReply || handleClick(item?.replyId || item?.id)" :disabled="!activeButtonReply">
                Trả lời
          ...
          <div v-if="viewingRepliesOf.includes(item.id)"
            ...
            <CommentTree :payload="replyChildren.get(item.id)!" :id="item.id" type="reply" :user="user"
              @reply="handleReply" />
          </div>
