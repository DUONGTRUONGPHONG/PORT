    ...
    <section class=" mb-8 px-3">
        <div class="" v-if="data?.length > 0">
            <div class="grid lg:grid-cols-2 gap-4">
                <article>
                    <div class="group">
                        <figure class="mb-2"><nuxt-link class="block relative w-full bg-[#eeeeee] pb-[56.25%] rounded-lg"
                                :to="`/${data[0]?.category?.code}/${data[0]?.code}`"><img :src="data[0]?.thumbnail" alt=""
                        ...
                        <header class="">
                            <h3
                                ...
                                <nuxt-link :to="`/${data[0]?.category?.code}/${data[0]?.code}`" v-html="data[0]?.title"></nuxt-link></h3>
...
const fetchData =async()=>{
        data.value=await store.article.fetchByCategoryIdWithPaging(props.categoryId,4)
        listData.value = data.value.slice(1,4);
}
await fetchData()
</script>